...
func SetupMux(ctx context.Context, cfg proxyConfig) *http.ServeMux {
	...
	err := gw.RegisterTodoServiceHandlerFromEndpoint(ctx, gwmux, cfg.backend, opts)
	if err != nil {
...
func SetupViper() *viper.Viper {
	...
	viper.AddConfigPath(".")
	viper.SetEnvPrefix("TodoService")
	...
	v := viper.GetViper()
	bindEnvVars(v, strings.NewReplacer(EnvVarSepChar, ViperKeySepChar), strings.ToUpper("TodoService"), ResponseHeaders)
	return v
}
